...
class PhotometricDistortWithBBox(object):
    def __init__(self, p=1.0):
        self.pd = [
            RandomContrast(),
            #
    ...
    def __call__(self, img, target, bbox_target):
        if random.random() < self.p:
            im = img.copy()
            im = PIL2CV()(im)
            im = self.rand_brightness(im)
            ...
            im = self.rand_light_noise(im)
            im = CV2PIL()(im)
            return im, target, bbox_target
        return img, target, bbox_target


class PhotometricDistortForInstanceSegmentation(object):
    def __init__(self, p=1.0):
        self.pd = [
            RandomContrast(),
            #
