...
visulizer = Visualizer(keys=['fake', 'real'])
...
for epoch in range(epoch_start, args.epochs):
    ...
    for (x_, y_) in train_loader:
        ...
        visulizer.display(x_[0].cpu(), 'real')
        print(G_result.shape)
        visulizer.display(G_result[0].detach().cpu().numpy(), 'fake')
    ...
    logger.log(key='D_real_loss', data=mean(D_fake_loss_list), show=True)
    logger.log(key='D_fake_loss', data=mean(D_fake_score_list), show=True)
    logger.log(key='G_train_loss', data=mean(G_train_loss_list), show=True)
    logger.save_log()
    logger.visualize()
